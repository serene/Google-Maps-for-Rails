...
The gem will use this container to include:

* the js files from map providers

* the custom js to display the map

Another kind of js is needed: the gem's js code.

Before v2.0, js files were magically included by the gem in your view. This required additional logic and didn't fit all expectations.
Now you have to include the files yourself which makes everything clearer.

For Rails 3.0.x or without assets pipeline, include the necessary files in your view:

    # base is always necessary
...
For Rails >= 3.1 with assets pipeline, same principle: add the required files to your manifest.
